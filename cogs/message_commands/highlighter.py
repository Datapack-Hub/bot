...
class Highlighter:
    class Database:
        with open(
            variables.full_path + "/cogs/message_commands/database.json",
            encoding="utf-8",
        ) as db:
        ...
        for index, char in enumerate(slice_string[0]):
            if len(strings) >= 1:
                ...
                if brackets_count == 0:
                    ...
                    if replace_slices:
                        slice_string[0] = slice_string[0].replace(
                            current_slice, replace_string.format(len(slices) - 1)
                        )
                    current_slice = ""
    ...
    def general_lexer(function):
        ...
        def cut_by_regex(matches_list, regex):
            function_elements[matches_list] = Highlighter.remove_duplicates(
                re.findall(regex, Highlighter.function_copy)
            )
        ...
        function_elements["selector_filter"] = Highlighter.brackets_slice(
            "[]", temp, True
        )
        ...
        for type, tokens in function_elements.items():
            for token in tokens:
                if type not in multiple_colors_types:
                    function = re.sub(
                        f"(?m)(?:(?<=\\s)|(?<=^)){token}(?:(?=\\s)|(?=$){raw_nbt_thing if type in 'text selector' else ''})",
                        f"{colors[type]}{token}",
                        function,
                    )
                else:
                    function = function.replace(
                        token, multiple_colors_types[type](token)
                    )
        return function
    ...
    def selector_filter(filters):
        ...
        highlighted = (
            temp[0]
            .replace("[", f"{colors['bracket0']}[{colors['key']}")
    ...
    def nbt(tag, brackets_depth=-1):
        ...
        for token, type in lexed:
            if token[:2] == "$(":
                ...
                highlighted += (
                    f"{colors['string']}{strings[int(token.replace('string', ''))]}"
                )
            else:
...
Highlighter.Database.multiple_colors_types = {
    "selector_filter": Highlighter.selector_filter,
    "nbt": Highlighter.nbt,
    "macro": Highlighter.macro,
}
