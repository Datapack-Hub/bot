...
class HighlighterCommand(commands.Cog, name="highlighter"):
    ...
    async def highlighter(self,inter: disnake.ApplicationCommandInteraction,):
        if inter.user.guild_permissions.manage_guild:
            ...
            await dph.log("`/highlighter` Command", f"{inter.user.name} viewed the highlighter interaction menu (Server: **{inter.guild.name}**)","orange",self)
            ...
            await dph.log("`/highlighter` Command", f"{inter.user.name} attempted to use the command (Server: **{inter.guild.name}**)","red",self)