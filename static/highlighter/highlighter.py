...
path = Path(variables.__file__).resolve().parent / "static" / "highlighter" / "database.json"
    ...
    def highlight(func, theme="default"):
        ...
        for index, token in enumerate(tokens):
            ...
            if token[0] == "\u200b":
                ...
                highlighted += (
                    (colors["macro_bf_command"] + "$" if "$" in token else "") + colors["command"] + raw_command
                )
